...
async def _login(client, outbox, email=EMAIL, password=PASSWORD):
    """Sign-in is challenge-only since SECURITY_AUDIT C-2 retired /v1/login."""
    return await sign_in(client, outbox, email=email, password=password)
...
async def test_access_token_carries_a_session_id(client, outbox):
    await _signup(client)
    token = (await _login(client, outbox)).json()["access_token"]
    assert _sid(token)


async def test_second_sign_in_ends_the_first_session(client, outbox):
    await _signup(client)
    first = _sid((await _login(client, outbox)).json()["access_token"])
    assert await _active(client, first) == [first]

    second = _sid((await _login(client, outbox)).json()["access_token"])
    assert second != first
...
async def test_evicted_session_cannot_refresh_its_way_back(client, outbox):
    await _signup(client)
    first_login = await _login(client, outbox)
    stale_refresh = _refresh_cookie(first_login)

    await _login(client, outbox)  # second device evicts the first

    replay = await client.post("/v1/token/refresh", cookies={"mxtng_refresh": stale_refresh})
    assert replay.status_code == 401


async def test_refreshing_does_not_evict_yourself(client, outbox):
    await _signup(client)
    login = await _login(client, outbox)
...
async def test_sessions_are_per_account_not_global(client, outbox):
    await _signup(client)
    await _signup(client, email="other@example.com")

    mine = _sid((await _login(client, outbox)).json()["access_token"])
    theirs = _sid((await _login(client, outbox, email="other@example.com")).json()["access_token"])

    # Someone else signing in must never touch my session.
    assert sorted(await _active(client, mine, theirs)) == sorted([mine, theirs])


async def test_logout_ends_the_session(client, outbox):
    await _signup(client)
    login = await _login(client, outbox)
    session_id = _sid(login.json()["access_token"])
...
async def test_password_reset_ends_every_session(client, outbox, monkeypatch):
    await _signup(client)
    session_id = _sid((await _login(client, outbox)).json()["access_token"])

    from mxtng_auth.settings import settings

    # The echo is opt-in since SECURITY_AUDIT M-10; enable it just for this test.
...
async def test_unknown_session_id_is_never_live(client, outbox):
...
async def test_a_pre_sessions_token_family_is_adopted_without_evicting_anyone(client, outbox):
    ...
    await _signup(client)
    current = _sid((await _login(client, outbox)).json()["access_token"])
